...
#   x <- lapply(X=x, FUN=cuttails)
   ...
   # Get densities.
   densities <- lapply(X=x, FUN=sm.density, h=.6, display="none")
...
#      col="grey95", border=NA)
   if (hbars) {
      ythick = seq(from=ceiling(BOT), to=floor(TOP), by=1)
#      ythin = seq(from=.5*ceiling(BOT/.5), to=.5*floor(TOP/.5), by=.5)
      segments(x0=LFT, x1=RGT, y0=ythick, y1=ythick, lwd=2, col="grey90")
